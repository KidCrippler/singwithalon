...
export const adminQueries = {
    ...
    create(username, passwordHash) {
        ...
        return { id: result.lastInsertRowid, username, password_hash: passwordHash, created_at: new Date().toISOString() };
    },
    exists() {
        const count = getDb().prepare('SELECT COUNT(*) as count FROM admins').get();
        return count.count > 0;
    },
};
// Queue queries
export const queueQueries = {
    add(songId, requesterName, sessionId) {
        const result = getDb().prepare('INSERT INTO queue (song_id, requester_name, session_id) VALUES (?, ?, ?)').run(songId, requesterName, sessionId);
        return getDb().prepare('SELECT * FROM queue WHERE id = ?').get(result.lastInsertRowid);
    },
    remove(id, sessionId) {
    ...
    markPlayed(id) {
        getDb().prepare('UPDATE queue SET status = ?, played_at = CURRENT_TIMESTAMP WHERE id = ?').run('played', id);
    },
    getAll() {
        return getDb().prepare('SELECT * FROM queue ORDER BY created_at ASC').all();
    },
    getGrouped() {
    ...
    countBySession(sessionId) {
        const result = getDb().prepare('SELECT COUNT(*) as count FROM queue WHERE session_id = ? AND status = ?').get(sessionId, 'pending');
        return result.count;
    },
    getBySession(sessionId) {
        return getDb().prepare('SELECT * FROM queue WHERE session_id = ? ORDER BY created_at ASC').all(sessionId);
    },
};
// Playing state queries
export const playingStateQueries = {
    get() {
        return getDb().prepare('SELECT * FROM playing_state WHERE id = 1').get();
    },
    update(updates) {
        const fields = [];
        const values = [];
...
export const sessionQueries = {
    upsert(sessionId, updates) {
        const existing = getDb().prepare('SELECT * FROM sessions WHERE session_id = ?').get(sessionId);
        if (existing) {
            const fields = ['last_seen = CURRENT_TIMESTAMP'];
            const values = [];
            ...
            getDb().prepare('INSERT INTO sessions (session_id, requester_name, is_projector, resolution_width, resolution_height, lines_per_verse) VALUES (?, ?, ?, ?, ?, ?)').run(sessionId, updates?.requester_name ?? null, updates?.is_projector ? 1 : 0, updates?.resolution_width ?? null, updates?.resolution_height ?? null, updates?.lines_per_verse ?? null);
        }
