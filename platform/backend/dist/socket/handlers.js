...
export function setupSocketHandlers(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('verse:set', ({ verseIndex }) => {
            if (!socketData.user?.isAdmin)
                return;
        ...
        // === Queue Events ===
        socket.on('queue:add', async ({ songId, requesterName }) => {
        ...
        socket.on('queue:present', ({ queueId }) => {
            if (!socketData.user?.isAdmin)
                return;
